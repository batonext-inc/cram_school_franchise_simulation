...
フランチャイズ塾のオーナーとなり、X1年4月からX6年3月までの60ヶ月を運営して総資金を最大化するシミュレーションゲームです。HTML/CSS/JavaScript（Canvas 2D）のみで完結し、ブラウザ（ローカルサーバー）だけで遊べます。
...
- **広告管理**: 校舎ごとに広告プラン（広告なし/ポスティング/リスティング/中吊り）を設定し、駅乗降客数を係数として翌月から効果反映。
- **新規開校**: 6拠点（新宿〜千葉）の中から未開校キャンパスを追加可能。資金不足の拠点には「資金不足」バッジを表示し選択不可。
